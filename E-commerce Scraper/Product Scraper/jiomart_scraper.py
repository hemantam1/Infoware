...
pincodes_df = pd.read_excel('Required files/pincodes_jiomart.xlsx')
options = Options()
...
def jio_pinc(p: int):
    driver.find_element(By.XPATH, '//*[@id="pincode_section"]/div[1]').click()
    driver.implicitly_wait(3)
    driver.find_element(By.XPATH, '//*[@id="rel_pincode"]').clear()
    driver.find_element(By.XPATH, '//*[@id="rel_pincode"]').send_keys(p)
    driver.implicitly_wait(3)
    driver.find_element(By.XPATH, '//*[@id="rel_pincode_form"]/div[1]/button[2]').click()
    driver.implicitly_wait(3)
...
if __name__ == "__main__":
    ...
    with open('Scrapped_Files/jiomart-products.csv', 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([datetime.datetime.now()])
        csvwriter = csv.DictWriter(f, fieldnames=headers)
        csvwriter.writeheader()
        items = jio_items_scraper(l, id, count, pincodes_df['Pin Code'][:2])
        for i in items:
